(() => {
  ...
  const USE_KEY = 'batch-os-free-batches-v1';
  const purchaseReturn = new URLSearchParams(location.search).get('purchase') === 'success';
  let access = { paid:false, tier:'trial', freeBatchLimit:5, foundingPriceUsd:19, regularPriceUsd:29, checkoutUrl:null };
  let pendingCheckout = false;
  let lastIdentity = '';

  const uses = () => Math.max(0, Math.floor(Number(localStorage.getItem(USE_KEY) || 0) || 0));
  const setUses = n => localStorage.setItem(USE_KEY, String(Math.max(0, Math.floor(n))));
  const left = () => Math.max(0, Number(access.freeBatchLimit || 5) - uses());
  ...
  function toast(msg,ms=2800){document.querySelector('.access-toast')?.remove();const t=document.createElement('div');t.className='access-toast';t.textContent=msg;document.body.appendChild(t);setTimeout(()=>t.remove(),ms)}
  ...
  function wrapCalculator(){
    const b=document.getElementById('calculate'); if(!b||b.dataset.paywallWrapped==='1')return; const original=b.onclick; b.dataset.paywallWrapped='1';
    b.onclick=async e=>{await refreshAccess();if(access.paid)return original?.call(b,e);if(uses()>=Number(access.freeBatchLimit||5)){openPaywall();return}const prior=state.plan;await original?.call(b,e);if(state.plan&&state.plan!==prior){setUses(uses()+1);updateUI();if(uses()>=Number(access.freeBatchLimit||5))setTimeout(openPaywall,900)}};
  }
  ...
  async function purchaseReturnFlow(){
    if(!purchaseReturn)return;toast('Payment received. Finalizing your Batch OS access…',5000);
    for(let i=0;i<12;i++){await refreshAccess();if(access.paid){const u=new URL(location.href);u.searchParams.delete('purchase');u.searchParams.delete('session_id');history.replaceState({},'',u.pathname+u.search+u.hash);toast('Batch OS unlocked. Welcome to Founding Access.',4500);return}await new Promise(r=>setTimeout(r,1250))}
    openPaywall();
  }

  async function watch(){const id=typeof state!=='undefined'?`${state.user?.email||''}|${state.token||''}`:'';if(id===lastIdentity)return;lastIdentity=id;await refreshAccess();if(pendingCheckout&&state.user&&!access.paid){pendingCheckout=false;beginCheckout()}}

  styles();build();wrapCalculator();wrapRecipeSaving();wrapSaveBatch();refreshAccess().then(()=>{updateUI();purchaseReturnFlow()});setInterval(watch,800);setInterval(()=>{wrapCalculator();wrapRecipeSaving();wrapSaveBatch()},1500);
})();
